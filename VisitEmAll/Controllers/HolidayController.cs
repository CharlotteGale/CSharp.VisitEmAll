...
public class HolidaysController : Controller
{
    ...
    public async Task<IActionResult> Like(int id, [FromForm] string? returnFragment)
    {
        var userId = HttpContext.Session.GetInt32("User_Id");
        ...
        var fragment = !string.IsNullOrEmpty(returnFragment) ? $"#{returnFragment}" : "";
        return Redirect(referer + fragment);
    }

    [HttpPost("/holidays/{id:int}/unlike")]
    public async Task<IActionResult> Unlike(int id, [FromForm] string? returnFragment)
    {
        var userId = HttpContext.Session.GetInt32("User_Id");
        if (userId == null) return Unauthorized();
